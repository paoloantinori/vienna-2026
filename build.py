...
locBtn.onAdd = function(m) {
  ...
  L.DomEvent.on(a,'click',function(e){
    ...
    window._locWatchId = navigator.geolocation.watchPosition(function(pos){
      ...
      var locIcon = L.divIcon({className:'',html:'<div style="width:22px;height:22px;background:#3b82f6;border:3px solid #fff;border-radius:50%;box-shadow:0 0 8px rgba(59,130,246,0.7);"></div>',iconSize:[22,22],iconAnchor:[11,11]});
      if (window._locMarker) window._locMarker.setLatLng([lat,lng]);
      else window._locMarker = L.marker([lat,lng],{icon:locIcon,zIndexOffset:10000,pane:'markerPane'}).addTo(map);
...
locBtn.addTo(map);
// Auto-start geolocation after map is ready
map.whenReady(function(){ setTimeout(function(){ a.click(); }, 500); });
